    ...
    if (
      ...
      if (lastRepoId) {
        ...
        const repoExists = repos.some(
          (r) => r.id === lastRepoId || r.fingerprint === lastRepoId
        )
        if (repoExists) {
          ...
          {!hasRepos ? (
            <div className='flex flex-col items-center justify-center p-8 text-center'>
              ...
              <p className='text-muted-foreground mb-4 max-w-sm text-xs'>
                {isLoggedIn
                  ? t`You have no repositories yet.`
              ...
              {isLoggedIn &&
                ...
                recommendations.filter((rec) => !subscribedRepoIds.has(rec.id))
                  .length > 0 && (
                  <>
            ...
            <div className='divide-y rounded-lg border'>
              {sortedRepositories.map((repo) => (
                ...
                >
                  <FolderGit2 className='text-muted-foreground h-5 w-5 shrink-0' />
